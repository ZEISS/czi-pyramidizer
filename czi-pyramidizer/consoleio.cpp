...
std::shared_ptr<libpyramidizer::IConsoleIo> CreateConsoleIo() { return std::make_shared<ConsoleIo>(); }
...
#endif
{
    ...
    if (stdoutFile != -2)
    {
        const intptr_t osf_handle = _get_osfhandle(stdoutFile);
        auto* consoleHandle = reinterpret_cast<HANDLE>(osf_handle);  // NOLINT
...
void ConsoleIo::SetColor(libpyramidizer::ConsoleColor foreground, libpyramidizer::ConsoleColor background)
{
#if CZIPYRAMIDIZER_WIN32_ENVIRONMENT
